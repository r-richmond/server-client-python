...
class UserDataTest(unittest.TestCase):
    ...
    def test_validate_import_file(self):
        valid, invalid = \
            TSC.UserItem.CSVImport.validate_file_for_import("test/assets/data/users_import_2.csv", UserDataTest.logger)
        assert len(valid) == 2, "Expected two lines to be valid, got {}".format(valid)
        assert len(invalid) == 3, "Expected 3 failures, got {}".format(len(invalid))

    def test_validate_usernames_file(self):
        users, errors = \
            TSC.UserItem.CSVImport.validate_file_for_import("test/assets/data/usernames.csv", UserDataTest.logger)
